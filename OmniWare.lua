print("Omniware Loading")
wait(1.0)
print("Loading")
wait(2.0)
print("Welcome to Omniware")
...
local cfglocation = "Omniwarecfg/" 
makefolder("Omniwarecfg") 
...
makefolder("Omniwarelua") 

local allluas = {} 

for _,lua in pairs(listfiles("Omniwarelua")) do 
	local luaname = GSUB(lua, "Omniwarelua\\", "") 
	INSERT(allluas, luaname) 
end 

RunService.RenderStepped:Wait() 

local gui = library:New("Omniware") 
...
luascripts:Element("Button", "load", {}, function() 
	loadstring(readfile("Omniwarelua\\"..values.luas["lua scripts"].lua.Scroll))() 
end) 
...
self:Element("Toggle", "Remove sleeves"), {}, function(tbl)
		if tbl.Toggle then
workspace.Camera.ChildAdded:Connect(function()
    for i,v in pairs(workspace.Camera.Arms:GetChildren()) do
        if v:FindFirstChild("Left Arm") then
            local leftArmSleeve = v:FindFirstChild("Left Arm"):FindFirstChild("Sleeve")
            local rightArmSleeve = v:FindFirstChild("Right Arm"):FindFirstChild("Sleeve")
            if leftArmSleeve then
                leftArmSleeve:Destroy()
            end
            if rightArmSleeve then
                rightArmSleeve:Destroy()
            end
        end
    end
end)
...
noclip:Element("Toggle", "Trail", {}, function(tbl)
		if tbl.Toggle then
...
movement:Element("ToggleKeybind", "edge bug") 
	local zuhnmode = Instance.new("ColorCorrectionEffect", workspace.CurrentCamera)
		movement:Element("Toggle", "zuhn mode", {}, function(tbl)
			if tbl.Toggle then
...
client:Element("Toggle", "Watermark", {}, function(tbl)
		if tbl.Toggle then	
    local colorScheme = Color3.fromRGB(254, 254, 254)
...
UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(43, 41, 43)), ColorSequenceKeypoint.new(0.48, Color3.fromRGB(249, 178, 189)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(43, 41, 43))}
...
UIGradient_2.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(43, 41, 43)), ColorSequenceKeypoint.new(0.48, Color3.fromRGB(249, 178, 189)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(43, 41, 43))}
...
UIGradient_3.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(43, 41, 43)), ColorSequenceKeypoint.new(0.47, Color3.fromRGB(249, 178, 189)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(43, 41, 43))}
...
UIGradient_4.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(43, 41, 43)), ColorSequenceKeypoint.new(0.47, Color3.fromRGB(249, 178, 189)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(43, 41, 43))}
...
local section = scriptColumn:AddSection("honzu's watermark")

section:AddBox({text = "Text 1", callback = function(val)
    TextGiven = val
end})

section:AddBox({text = "Text 2", callback = function(val)
    TextGiven2 = val
end})

section:AddColor({text = "Color Scheme", callback = function(color)
    colorScheme = color
end})
end
end)
											...
											if #Hits == 0 then 
												...
												if values.rage.exploits['custom tap'].Toggle and values.rage.exploits['custom tap'].Active then
													...
													elseif values.rage.aimbot["automatic fire"].Dropdown == "hitpart" then
														...
														game.ReplicatedStorage.Events.HitPart:erver(unpack(Arguments))
														if values.rage.exploits['custom tap'].Toggle and values.rage.exploits['custom tap'].Active then
														for chingchong = 2, values.rage.exploits['tap amount'].Slider do
															Client.firebullet()
															local Arguments = {
